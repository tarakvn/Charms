...
class _HomePageState extends State<HomePage> {
  ...
  // تابع کمکی برای بزرگ کردن حرف اول کلمه
  String _capitalize(String s) => s.isEmpty ? s : s[0].toUpperCase() + s.substring(1);

  void _castSpell(String text) async {
    // 1. تبدیل به حروف کوچک
    // 2. حذف کوتیشن‌ها و علائم نگارشی که گاهی سیستم گفتار اضافه می‌کند
    final String normalizedText = text.toLowerCase().replaceAll(RegExp(r'[^\w\s]'), '').trim();

    for (var spell in allSpells) {
      // حالا هم کلمه شنیده شده تمیز است و هم طلسم ما
      if (normalizedText.contains(spell.incant.toLowerCase())) {
    ...
    // نمایش متنی که واقعاً شنیده شده (بدون کوتیشن‌های اضافه)
